...
- [x] Test signed headers built from an already verified hash without buffering bytes again.
- [x] Test local file size verification before the request.
- [x] Test one bounded `PUT /v1/artifacts` request with locator, size, type, hash, tenant, client, timestamp, and nonce headers.
- [x] Test explicit outcomes for stored, already stored, capacity deferred, conflict, and ambiguous server responses.
- [x] Implement the minimal streaming client and run all client tests.
- [x] Commit as `feat(client): stream signed temporary artifacts`.
...
- [x] Test atomic reservation, identical retry, conflicting locator reuse, stale-upload recovery, successful availability, and publication claim.
- [x] Add `artifact_uploads` with tenant-scoped locator identity, checksum, size, media type, state, expiry, and timestamps.
- [x] Add a database trigger that rejects active unclaimed uploads before the configured R2 reject limit.
- [x] Ensure incomplete and unclaimed uploads are visible to bounded cleanup.
- [x] Commit as `feat(worker): reserve temporary artifact uploads`.
...
- [x] Test unknown client, wrong tenant, stale timestamp, bad signature, invalid locator, unsupported media type, invalid size, capacity rejection, checksum mismatch, idempotent retry, locator conflict, and verified success.
- [x] Authenticate the signed declared hash without buffering the request body; rely on R2 `sha256` validation for the streamed bytes.
- [x] Restrict locators to `temporary/<tenant>/.../<sha256>.<extension>` and media types to an explicit allowlist.
- [x] Verify the returned R2 object and its `head` metadata before marking the upload available.
- [x] Route `PUT /v1/artifacts` without waking delivery work.
- [x] Commit as `feat(worker): accept verified temporary artifacts`.
...
- [x] Test rejection when a temporary object is absent or differs in checksum, size, media type, or tenant.
- [x] Test successful intake atomically marks matching upload rows claimed and stores temporary artifacts as available.
- [x] Keep external and live artifact behavior unchanged.
- [x] Commit as `feat(intake): require verified temporary artifacts`.
