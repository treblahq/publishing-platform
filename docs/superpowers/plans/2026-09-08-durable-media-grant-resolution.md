...
- [x] Write a real-SQLite fixture loading every actual migration, with an enabled Openings tenant, publication, `social.buffer` delivery in `processing`, available PNG artifact, unsafe-to-delete reference, explicit enabled adapter control, and unexpired approval. Write the accepting assertion first:
...
- [x] Run `npm run test:prepared -- apps/worker/src/artifacts/d1-public-media-grants.test.ts`; confirm the missing-module failure, then assertion failures against a deny-only stub after adding the schema.
- [x] Add the schema below. No public API or grant-writing service is part of this slice.
