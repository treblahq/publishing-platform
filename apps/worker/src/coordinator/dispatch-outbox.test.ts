...
describe('D1 outbox dispatcher', () => {
  it('does not mark work dispatched when queue send fails', async () => {
    ...
    await expect(dispatchOutbox({
      listDue: () => Promise.resolve([{ id: 'outbox-1', tenantId: 'openings', deliveryId: 'delivery-1' }]),
  ...
  it('safely sends the same outbox row again after a post-send crash', async () => {
    ...
    let sends = 0;
    let marks = 0;
    const row = { id: 'outbox-1', tenantId: 'openings', deliveryId: 'delivery-1' };
    const store = {
      listDue: () => Promise.resolve([row]),
      markDispatched: () => {
        marks += 1;
