...
async function activate(database: Database, tenant: string, entity: WebEntityRevision, objectKey: string) {
  // Check the conflicting row inside the write; a preceding SELECT races other activations.
  const activated = await database.prepare(`INSERT INTO web_entity_manifests
    ...
    WHERE web_entity_manifests.revision != excluded.revision
       OR web_entity_manifests.content_sha256 = excluded.content_sha256
    RETURNING content_sha256`)
    .bind(tenant, entity.kind, entity.id, entity.revision, entity.status, entity.title,
      entity.summary ?? null, entity.canonicalPath, entity.contentSha256, objectKey).first();
  if (!record(activated)) throw new Error('Revision reuse has different content');
}
