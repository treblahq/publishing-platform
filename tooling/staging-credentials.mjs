...
export function assertStagingCredentials(environment) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    assertStagingCredentials(process.env);
    process.stdout.write('Staging credentials passed structural checks.\n');
  } catch (error) {
    process.stderr.write(`${error instanceof Error ? error.message : 'Staging credential preflight failed'}\n`);
    process.exitCode = 1;
  }
}
