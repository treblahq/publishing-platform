...
export function hydrateProductionConfig(config, environment = process.env, configDirectory = process.cwd()) {
  ...
  const database = hydrated?.env?.production?.d1_databases?.[0];
...
async function main() {
  ...
  const hydrated = hydrateProductionConfig(config, process.env, dirname(resolve(source)));
